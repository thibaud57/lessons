...
Chaque leçon est une fiche de révision dense : concepts clés, pièges, dépréciations, blocs de code. Pas de tutoriel pas à pas — de la substance directement exploitable en entretien ou en production.
...
lessons/
├── index.yaml            # Index de toutes les leçons (gitignored)
├── index-example.yaml    # Format attendu de l'index (exemple vide)
├── {techno}/             # Leçons par techno (gitignored)
│   └── {lecon}.md
└── ...
...
Créer l'index vide pour commencer :
